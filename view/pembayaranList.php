                ...
                <?php if (!empty($pembayarans)): ?>
                    <?php foreach ($pembayarans as $p): ?>
                        <tr class="border-t hover:bg-[#FFF3E4]">
                            <td class="p-3"><?= $p['id_pembayaran']; ?></td>
                            <td class="p-3"><?= htmlspecialchars($p['nama_client']); ?></td>
                            <td class="p-3"><?= ucfirst($p['jenis']); ?></td>
                            <td class="p-3">
                                <?php if (!empty($p['bukti_pembayaran'])): ?>
                                    <a href="/project web/uploads/<?= $p['bukti_pembayaran']; ?>" target="_blank" class="text-blue-500 hover:underline">
                                        Lihat Bukti
                                    </a>
                                <?php else: ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="6" class="p-4 text-center">Tidak ada data pembayaran.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
